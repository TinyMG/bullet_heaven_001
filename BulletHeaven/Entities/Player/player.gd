...
func _physics_process(delta: float) -> void:
	...
	if _nearest_enemy_timer <= 0.0 or (_cached_nearest_enemy != null and not is_instance_valid(_cached_nearest_enemy)):
		_cached_nearest_enemy = _find_nearest_enemy()
		_nearest_enemy_timer = 0.2

	# Flip sprite handling
	if velocity.x != 0:
		sprite.flip_h = velocity.x < 0
	elif current_anim_state == AnimState.SHOOT:
		if _cached_nearest_enemy != null and is_instance_valid(_cached_nearest_enemy):
...
func _on_fire_timer_timeout() -> void:
	if _cached_nearest_enemy == null or not is_instance_valid(_cached_nearest_enemy):
		return
	_fire_at(_cached_nearest_enemy)

func _find_nearest_enemy() -> Node2D:
	var enemies = get_tree().get_nodes_in_group("Enemy")
	var closest: Node2D = null
	var closest_dist_sq: float = 250000.0  # 500.0 squared for Max targeting range
