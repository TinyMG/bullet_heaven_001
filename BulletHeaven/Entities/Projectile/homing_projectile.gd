...
var _cached_nearest: Node2D = null
...
func _physics_process(delta: float) -> void:
	...
	if _retarget_timer <= 0.0:
		_cached_nearest = _find_nearest_enemy()
		_retarget_timer = 0.2

	if _cached_nearest and is_instance_valid(_cached_nearest):
		var desired_dir = (_cached_nearest.global_position - global_position).normalized()
