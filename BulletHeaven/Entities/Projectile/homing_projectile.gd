...
func _physics_process(delta: float) -> void:
	# Home toward nearest enemy (cached to optimize performance)
	_target_timer -= delta
...
func _find_nearest_enemy() -> Node2D:
	...
	var closest_dist_sq: float = 160000.0 # 400.0^2
	for enemy in enemies:
		# Use distance_squared_to to avoid expensive square root calculations
