...
var _target_interval: float = 0.2
...
func _physics_process(delta: float) -> void:
	# Optimization: Only re-evaluate nearest target periodically
	_target_timer -= delta
	if _target_timer <= 0.0:
		_target = _find_nearest_enemy()
		_target_timer = _target_interval
	elif _target != null and not is_instance_valid(_target):
		_target = null
...
func _find_nearest_enemy() -> Node2D:
	...
	# Optimization: Use squared distance to avoid expensive square root calculations
