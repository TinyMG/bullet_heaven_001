...
func _physics_process(delta: float) -> void:
	# Update target periodically to avoid expensive queries every frame
	_target_update_timer -= delta
	if _target_update_timer <= 0.0 or (_target != null and not is_instance_valid(_target)):
		_target = _find_nearest_enemy()
		_target_update_timer = 0.2

	# Home toward cached nearest enemy
...
func _find_nearest_enemy() -> Node2D:
	...
	var closest_dist_sq: float = 160000.0 # 400.0 squared for performance
