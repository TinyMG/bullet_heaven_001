...
# Optimization: Cache nearest enemy to avoid per-frame get_nodes_in_group overhead
...
func _physics_process(delta: float) -> void:
	# Home toward nearest enemy (cached to avoid per-frame get_nodes_in_group overhead)
	_target_timer -= delta
	if _target_timer <= 0.0 or not is_instance_valid(_cached_target):
		_cached_target = _find_nearest_enemy()
		_target_timer = TARGET_REFRESH_RATE
